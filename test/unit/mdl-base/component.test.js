...
import MDLBase from '../../../packages/mdl-base';

class FakeComponent extends MDLBase {
  get root() {
    return this.root_;
  }
...
test('provides a static attachTo() method that returns a basic instance with the specified root', t => {
  ...
  const b = MDLBase.attachTo(root);
  t.true(b instanceof MDLBase);
  t.end();
});
...
test('provides a default initialSyncWithDOM() no-op if none provided by subclass', t => {
  t.doesNotThrow(MDLBase.prototype.initialSyncWithDOM.bind({}));
  t.end();
});
